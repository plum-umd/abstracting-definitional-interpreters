...
(define (ReaderT M)
  (with-monad M
    (monad
      ...
      ; effects
      ((compose
        (if (hash-has-key? (monad-effects M) 'reader)
            ...
            (λ (h) (hash-set h 'reader (monad-reader
                                        ...
                                        (λ (r′ xM)
                                          (λ (r)
                                            (xM r′)))))))
        (if (hash-has-key? (monad-effects M) 'writer) 
            ...
            (λ (h) (hash-set h 'writer (monad-writer
                                        ...
                                        (λ (xM)
                                          (λ (r) (hijack (xM r)))))))
            (λ (h) h))
        (if (hash-has-key? (monad-effects M) 'state)
            ...
            (λ (h) (hash-set h 'state (monad-state
                                       ...
                                       (λ (s)
                                         (λ (r)
                                           (put s))))))
            (λ (h) h))
        (if (hash-has-key? (monad-effects M) 'nondet)
      ...
      ; properties
      ((compose
        (if (hash-has-key? (monad-properties M) 'monoid-functor)
...
(define (StateT O M)
  ...
  (monad
    ...
    ; properties
    ((compose
      (if (and (hash-has-key? (monad-properties M) 'monoid-functor)
               (monoid? O)) 
